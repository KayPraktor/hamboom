...
- **گام بعدی:** ۳٫۷ — ★★ G-1الف: تستِ دو-نمونه‌ای با بومِ واقعی (رندرِ حضور + re-project).
  **فازهای ۰، ۱ و ۲ کامل‌اند** (`packages/ydoc-schema` تمام است) و **گام‌های ۳٫۱ تا
  ۳٫۶ هم تمام‌اند**: دو بومِ واقعی در مرورگر همگام می‌شوند، همدیگر را می‌بینند و
  تصویر رد و بدل می‌کنند؛ `Ctrl+Z` فقط کارِ خودِ کاربر را برمی‌گرداند و به همتا هم
  می‌رسد (`pnpm --filter @hamboom/canvas-sync test:e2e` → ۱۷/۱۷).
