...
- **★★ M6 (`admin`) — فاز ۰–۷ تمام و **تایید شده** (۷: ۱۴۰۵/۰۷/۰۳)؛ ⏭ قدمِ بعد فاز ۸ (E2E و سخت‌سازی) — هنوز شروع نشده، با تاییدِ مالک.**
  ★★ **فاز ۷:** [`touchLastSeen`](apps/api/src/routes/auth.ts) در ورود **و** refresh — گلو در خودِ `WHERE` (حداکثر یک UPDATE هر ۱۵ دقیقه) و عمداً **بیرونِ** تراکنشی که `FOR SHARE` دارد
  (داخلش روی PGِ زنده `40P01 deadlock detected` داد: دو refreshِ هم‌زمانِ یک کاربر) ·
  [`GET /admin/stats`](apps/api/src/services/admin-stats.ts) با **SQLِ خالص** و هر تجمیع `::bigint` (B-2)، سطلِ روزانه = **روزِ تهران** نه UTC (سطلِ UTC برچسبِ جلالی را ۳٫۵ ساعت دروغ می‌کرد؛ ۴ از ۱۶ سطلِ این ماشین)،
  «تیم‌ها به تفکیکِ پلن» از `teams` شروع می‌شود و `subscriptions` را LEFT JOIN می‌کند (وگرنه ۱۸ از ۲۰ تیم بی‌صدا حذف می‌شدند)، سریِ **صفر‌پُر** ·
  نمودارِ [SVGِ خانگی](apps/web/src/panel/BarChart.tsx) با **صفر dep** (ورودی +۵۱۴ B، chunkِ پنل ۳۱٫۵→۴۲٫۳KB، صفر میزبانِ خارجی) ·
  [`GET /admin/system`](apps/api/src/services/system-status.ts) با **۸ چکِ موازی** و مهلتِ per-probe: DB · سه باکت با **`iteratePrefix`** (نه `headObject` — آن روی باکتِ **ناموجود** هم `null` می‌دهد و چکِ سلامت را سبزِ دروغین می‌کرد) ·
  Redis با **RESP PINGِ `node:net`** و صفر وابستگیِ نو · سنِ پشتیبان (آستانه ۳۰ ساعت) · **اختلافِ ساعتِ PG↔api** · و `reconcile` با **سه** معنیِ جدا برای «هرگز اجرا نشده» ·
  نمای فقط‌خواندنیِ `feature_flags` · `sdk:contract` **۴۹/۴۹**.
  ★★ **و یک باگِ تولیدِ فاز ۳ همین‌جا پیدا و رفع شد:** `stepUpFresh` با `age >= 0` هر جلو‌بودنِ ساعتِ دیتابیس (اندازه‌گیری: **۰–۲ms**) را «کهنه» می‌شمرد ⇒ staff کد را وارد می‌کرد و عملِ بعدی **باز ۴۲۸** می‌گرفت؛ حلقه‌ی بی‌پایان.
  رواداریِ صریحِ ۲ ثانیه، و `/admin/system` حالا خودِ اختلاف را نشان می‌دهد. ⚠️ این را دو شکستِ «گذرا»ی `sdk:contract` لو داد — «گذرا» یک تشخیص نیست.
  ⚠️ **سه سبزِ دروغین در گیت‌های خودم** با شکستنِ عمدی گرفته شد (چکِ قرارداد `::bigint` را نمی‌دید چون mapper با `Number()` نجاتش می‌داد · سطلِ UTC را هم نمی‌دید چون مجموع و برچسب هر دو همان می‌مانند · و probeِ بن‌بست سناریوی دومش را اصلاً اجرا نمی‌کرد). جدول در [PROGRESS](PROGRESS-M6-admin.md).
  ⊕ **ایندکسِ آمار عمداً اضافه نشد:** روی جدولِ ۳۰۰هزار ردیفی سریِ ۳۰ روزه **۱۶ms** بدونِ ایندکس و ۵ms با آن (۳٫۶×) به قیمتِ ۶٫۱MB روی جدولِ ۱۳MB — تریگرِ بازنگری: عبور از ~۲۰۰ms.
