...
در گام ۱٫۲ (M2) دیده شد که `pnpm lint` **سبز گزارش می‌کند در حالی که واقعاً می‌افتد**.
علت با آزمایش پیدا شد، نه حدس: کشِ turbo **نبود**؛ یک پروکسیِ بهینه‌سازِ خروجی
(`rtk`، از تنظیماتِ سراسریِ توسعه‌دهنده و بیرون از این ریپو) روی دستورهای شناخته‌شده‌ی
pnpm سوار می‌شود و خروجی را خلاصه می‌کند — و یک اجرای شکست‌خورده را به
«ESLint: No issues found» با **exit 0** تبدیل می‌کرد.

بازتولیدِ قطعی، با یک فایلِ عمداً خراب:

| دستور | خروجی | exit |
|---|---|---|
| `pnpm lint` | «No issues found» | **۰** ❌ |
| `npx turbo run lint` | خطای واقعی | ۱ ✅ |
| `rtk proxy pnpm lint` | خطای واقعی | ۱ ✅ |

[`scripts/verify.mjs`](scripts/verify.mjs) سه کار می‌کند: از راهِ `turbo`ِ مستقیم
اجرا می‌کند (نه اسکریپت‌های `pnpm`ِ بازنویسی‌شده)، تصمیم را **فقط با کدِ خروجِ فرایند**
می‌گیرد نه با متنِ خروجی، و `--force` می‌زند تا ثابت شود دستورها **اجرا شدند**.
خودِ گیت با یک خطای عمدی آزموده شد و گرفتش.
...
- **گام بعدی:** ۱٫۳ — probeِ متنِ همزمان. فاز ۰ کامل؛ ۱٫۱ (دروازه) و ۱٫۲ سبز شدند.
