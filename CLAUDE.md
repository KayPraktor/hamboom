...
```bash
node scripts/verify.mjs > verify.log 2>&1; grep -ic "out of memory" verify.log
```

عددِ غیرِ صفر یعنی **محیط**، نه کد → چند تب مرورگر ببند یا page file را بزرگ‌تر کن.
صفر یعنی یک شکستِ واقعی. این تفکیک را حدس نزن؛ همان درسِ سبزِ دروغینِ گام ۱٫۲.

## وضعیت فعلی

- **ماژول فعال:** M2 — `realtime-sync` (فاز ۲ در جریان)
- **گام بعدی:** ۳٫۱ — اسکلت آداپتور + چرخه‌ی عمر + نگهبانِ echo (★ قلبِ M2).
  **فازهای ۰، ۱ و ۲ کامل‌اند** — `packages/ydoc-schema` تمام است: codecِ هر پنج
  ریشه، migrationِ آزموده، و هر ۷ پیامِ پروتکل.
