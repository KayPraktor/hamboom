...
### ⏳ پیامک: سرویس انتخاب شد، ولی **قالب** بلاک است (۱۴۰۵/۰۶/۱۸)

  **sms.ir** انتخابِ مالک است و حساب آماده. ⛔ **ولی قالبِ تاییدشده نمی‌دهند چون «سایت بالا
  نیست»** — و سایت بالا نیست چون لانچ نشده‌ایم.

  ★★ **قوی‌ترین راهِ حل با کارِ فاز ۲ ساختنی است:** یک صفحه‌ی معرفی روی دامنه (دامنه ✅،
  VMِ آروان ✅، ایمیجِ nginx+SPA ✅). فهرستِ کاملِ پنج راه و **سه چیزی که برای شروعِ فازِ ۴٫۵
  لازم است** (`templateId` · نامِ **دقیقِ** پارامتر · یک شماره‌ی واقعی برای تماسِ زنده) در
  [`TODO-M5-infra.md`](TODO-M5-infra.md).

  ⚠️⚠️ **یک کلیدِ APIِ sms.ir در گفت‌وگو فرستاده شد و باید باطل شود.** کلیدِ تازه هرگز در
  چت نیاید — فقط `.env`ِ محلی و `.env.production`ِ روی VM.
