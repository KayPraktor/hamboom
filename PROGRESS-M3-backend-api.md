...
**چه شد:** سه پالایشِ رابط پیش از فاز ۱۱، به‌خواستِ مالک — همه **فقط `apps/web`**: **صفر لمسِ M1/M2/shared-types،
صفر ADR**، همه از موجود reuse (ADR-024). **★ لیستِ منو پیش از ساخت به تاییدِ مالک رسید** (اصلِ «چیزی بی‌پشتوانه
نگذار» — هر آیتم یک قابلیتِ واقعی).

| بخش | چه شد | اثبات در مرورگر |
|---|---|---|
| **۱ — بومِ تمام‌صفحه** | `board-shell` → `position:fixed; inset:0` (کلِ ویوپورت، روی هدرِ سراسری) — خودش را از پدینگِ `app-main` جدا می‌کند، **بی‌دست‌زدن به `RootLayout`**. خوشه‌ی کنترلِ جمع‌وجورِ **بالا-شروع** (بازگشت + عنوان + وضعیت + `⋯`) شناور، بومِ excalidraw کلِ صفحه | اندازه‌گیریِ DOM: shell = کلِ ۹۶۱×۹۱۴، بوم پر، خوشه ۴۲px بالا-شروع، هدرِ سراسری پوشیده |
| **۲ — منوی `⋯`** | فقط قابلیت‌های واقعی: شبکه (`gridModeEnabled`)/چسبیدن (`objectsSnapModeEnabled`)/مکان‌نمای همکاران (گیتِ `PeerCursors`) · واگرد/ازنو (همان `adapter.undo`ِ Ctrl+Z) · تغییرِ نام (`PATCH /boards`) · حذف (`DELETE /boards`، فقط owner) | هر سه toggle state درست flip؛ **rename سرتاسر: `PATCH→۲۰۰`→refetch→عنوان به‌روز شد**؛ آیتم‌ها = دقیقاً لیستِ تاییدشده |
| **۳ — سایدبارِ داشبورد** | آیکون‌های خطیِ تک‌رنگِ **هم‌تراز** جای ایموجیِ درون‌متن؛ برچسبِ بخشِ **«فضاها»**؛ سرتیترِ تیمِ روشن‌تر (۱۳px fg + هاورِ اکسنت)؛ تورفتگیِ ۸px فولدر | DOM: هر ۴ آیتم آیکونِ هم‌تراز، برچسب «فضاها»، دو سرتیترِ تیم، فولدرِ تورفته؛ + screenshot |

**تصمیم‌ها و نکات:**
- **★ Duplicate عمداً حذف شد** (تاییدِ مالک): endpointِ `POST /boards/:id/duplicate` **هست** ولی **content-less**
  است — فقط متادیتا/عنوانِ «(کپی)» را کپی می‌کند، بی‌Y.Doc (کامنتِ خودِ [`boards.ts`](apps/api/src/routes/boards.ts):
  «محتوا = فاز بعد»). «تکثیر»ی که بوردِ **خالی** بسازد گمراه‌کننده است، پس مثلِ Export/History/Templates/Comment (فاز ۱۰)
  نگذاشتیم — **نه حتی «به‌زودی»**. **★ (Star)** هم به‌خواستِ مالک نیامد گرچه `favorite` واقعی است.
- **★ همه از موجود reuse شد** (ADR-024): `Toolbar`/`ZoomControl`/`PeerCursors`ِ canvas-core، `adapter.undo`
  (UndoScope)ِ canvas-sync، استایلِ `.card-menu`ِ `BoardCardMenu`. تنها منطقِ نو: هوکِ `useRenameBoard` کنارِ بقیه‌ی
  `boards-queries` (حذف = همان `useTrashBoard`ِ موجود).
- **★ شبکه/چسبیدن/مکان‌نما محلی‌اند** (با همتاها هم‌گام نمی‌شوند): سینک فقط `elements` را می‌برد، `appState` اصلاً
  sync نمی‌شود — پس این‌ها تنظیمِ نمایشِ **per-user** اند، مثلِ منوی View میرو. منو صادقانه با همین معنا برچسب می‌خورد.
- **★ واگرد/ازنوی منو = همان مسیرِ `Ctrl+Z`:** getterِ `adapter.undo` خودش بعدِ undo/redo `flushRemote` می‌زند (پس
  همتا هم می‌بیند)؛ منو فقط `undoScopeRef.current.undo()/redo()` را صدا می‌زند. صفر منطقِ نو، همان دسته که `bindUndoShortcuts` می‌بندد.
- **z-index خوشه ۱۲** (بالای روکش‌های canvas-core: نوار/زوم=۶، مکان‌نما=۷، منوی راست‌کلیک=۱۰)؛ منو z-۵۰. **RTL:**
  خوشه با logical properties (`inset-inline-start`)، منو رو به inline-end (فضایِ بازِ بوم)، نوتیس با `inset-inline:0 +
  margin auto` وسط. ⚠️ **بدونِ `letter-spacing`** روی برچسبِ فارسیِ سایدبار (خطِ متصل می‌شکند).
- ⚠️ **screenshot در این محیط پیوسته time-out شد** (بومِ excalidraw روی GPU composite؛ درسِ نشستِ قبل)، پس اثبات با
  **اندازه‌گیریِ DOM + کلیکِ واقعی + بازرسیِ شبکه** انجام شد. ⚠️ **realtimeِ زنده (undo/دوتب) این نشست اجرا نشد** چون
  پورتِ هاستِ redis در رنجِ excludedِ ویندوز افتاد (تله‌ی CLAUDE.md، محیطی نه کد)؛ undo همان مسیرِ اثبات‌شده‌ی Ctrl+Z است.
- **api با `OTP_DEV_FIXED_CODE=123456` ری‌استارت شد** (کدش دست‌نخورده) تا لاگینِ تست ممکن شود — درِ dev، نه تغییرِ محصول.

**اثباتِ نهایی:** `pnpm verify` سبز (هر ۸ گیت). در مرورگر با api/realtimeِ واقعی: بومِ تمام‌صفحه، منوی کامل، سه
toggle، و rename سرتاسر اثبات شد.
**قدمِ بعد:** فاز ۱۱ (ظرفیت + سخت‌سازی + تحویل) — فاز ۱۰ به تعویق.
