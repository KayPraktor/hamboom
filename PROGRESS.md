...
**گام فعلی: گام ۴٫۲ (نوار ابزار) تمام → بعدی ۴٫۳ (پنل‌ها و منوی راست‌کلیک)**
پله‌ی [ADR-003](ARCHITECTURE_DECISIONS.md#adr-003): **A** (بسته npm، بدون patch)

**۴۴۴ تست سبز** (۴۴ shared-types، ۳۵۸ canvas-core، ۴۲ i18n). `typecheck` · `lint` ·
...
## قدم بعدی — گام ۴٫۳ (پنل‌ها و منوی راست‌کلیک)

نوار ابزار (۴٫۲) تمام. طبق TODO گام ۴٫۳:
- پنلِ استایل وقتی چیزی انتخاب است (رنگ، ضخامت، شفافیت، لایه، قفل) — `StylePanel`
  از گام ۳٫۳ هست؛ گسترش و اتصالِ کامل.
- منوی راست‌کلیکِ RTL: کپی، پیست، تکثیر، حذف، گروه، لایه، قفل، «کپی به‌عنوان تصویر».
- کنترلِ zoom + «برازش با صفحه» + درصد با ارقام فارسی؛ mini-map؛ نوارِ وضعیت
  (`ConnectionState`/`SaveState`) با متنِ فارسی از `@hamboom/i18n`.
