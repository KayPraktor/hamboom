...
**گام فعلی: ★ فاز ۴ (رابط RTL) کامل — به‌جز mini-map که موکول شد → بعدی فاز ۵ (گام ۵٫۱)**
پله‌ی [ADR-003](ARCHITECTURE_DECISIONS.md#adr-003): **A** (بسته npm، بدون patch)

**۴۹۴ تست سبز** (۴۴ shared-types، ۴۰۸ canvas-core، ۴۲ i18n). `typecheck` · `lint` ·
...
## قدم بعدی — فاز ۵ (تعامل و صیقل)، گام ۵٫۱

★ **فاز ۴ کامل** (به‌جز mini-map که موکول شد). طبق TODO گام ۵٫۱:
- انتخابِ چندتایی با کادر و `Shift+Click`؛ گروه‌بندی (`Ctrl+G`/`Ctrl+Shift+G`).
- راهنمای هم‌ترازی هنگام درگ + snap به عناصر و شبکه.
- ابزارِ هم‌ترازی (چپ/راست/وسط، توزیع یکنواخت).
- **تغییرِ لایه (جلو/عقب/جلوترین/عقب‌ترین) با fractional index** ([ADR-007](ARCHITECTURE_DECISIONS.md#adr-007)) —
  **همان‌جا هر دو سطحِ ۴٫۳ (منو + پنل استایل) به این تابعِ واحد وصل می‌شوند** (از coming-soon درمی‌آیند).
