...
### ★ دو سوالِ بازِ گام ۴٫۴ (حضور) — هنوز جواب داده نشده

- [ ] **Q1 — re-project شدنِ مکان‌نما/هاله‌ی همتا هنگام zoom/panِ محلی (تست‌نشده).**
      وقتی کاربرِ محلی zoom/pan می‌کند **بدون پیامِ جدید از همتا**، آیا مکان‌نما و
      هاله‌ی انتخابِ همتا دوباره پروجکت می‌شوند (با محتوا جابه‌جا می‌شوند) یا سرِ
      جای قبلیِ **صفحه** می‌مانند؟ `PeerCursors`/`PeerSelections` هنگام رندر `project`
      را با `appState`ِ **فعلی** حساب می‌کنند — ولی فقط اگر re-render شوند.
      **فرضیه (آزموده نشده):** در دمو **zoom** re-render می‌دهد (`onScrollChange →
      setZoom`) پس درست re-project می‌شود؛ ولی **panِ خالص** (بدون تغییر zoom) هیچ
      state ای را عوض نمی‌کند → احتمالاً re-render نمی‌شود و مکان‌نمای همتا تا
      پیام/zoomِ بعدی سرِ جای قبلیِ صفحه می‌ماند. **باید در مرورگر آزموده شود**؛
      فیکسِ احتمالی: در دمو روی `onScrollChange` یک state برای scroll هم بگذار
      (M2 binder به‌هرحال با هر تغییرِ viewport باید re-render بدهد).
- [ ] **Q2 — تاییدِ حضور با کانالِ دومِ `BroadcastChannel` بود، نه دو نمونه‌ی واقعی.**
      بازِ دومِ تب با policy بسته بود، پس هر دو جهت با یک `BroadcastChannel`ِ دوم در
      همان تب تایید شد. مکانیزمِ کانال یکسان است، ولی رفتارِ **دو موتورِ واقعی**
      (viewportِ مستقل، re-project، تعاملِ همزمان) آزموده نشد. → **ذیلِ G-1 ثبت شد**
      (نه ورودیِ جدا): «تستِ دو-نمونه‌ای با binder واقعی» در گام ۶٫۱ باید **رندرِ
      حضور** (مکان‌نما/هاله/follow) را هم پوشش دهد، نه فقط echo/round-tripِ آداپتور.

**گپ‌های ثبت‌شده برای M2/6٫۱** (در `sync/README.md` و TODO گام ۶٫۱):
G-1 تست دو-نمونه‌ای با binder واقعی **(+ رندرِ حضور — Q2 بالا)** · G-2 تست رگرسیون
هش پیکسلی جهت متن · **Q1 (re-project حضور هنگام pan)**.
