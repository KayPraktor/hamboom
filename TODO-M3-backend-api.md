...
### گام ۸٫۳ — داشبورد + تیم (UI) — ✅ معیارِ پذیرش (۱۴۰۵/۰۶/۱۰)
- [x] فهرست/ساخت/نشان/جستجوی بورد (TanStack Query) + صفحه‌ی تیم: **اعضا/دعوت/تغییرِ نقش/حذف**.
- [x] **فولدر + سطلِ بازیافت** ✅ (۱۴۰۵/۰۶/۱۰) — ریلِ پیمایشِ per-team (`FolderNav`): فولدر
      (ساخت/تغییرِنام/حذفِ نرم) + جابه‌جاییِ بورد (منوی ⋯، `BoardCardMenu` با `boards.get`→فولدرهای
      همان تیم) + **سطلِ بازیافت**. برای لیستِ سطل، `GET /boards?trashed=true` به api افزوده شد
      (گیتِ owner مثلِ `assertDeletedBoardOwner`؛ **shared-types دست‌نخورد**) + `trashed?` در `sdk`.
      منطقِ خالصِ انتخاب→فیلتر در [`selection.ts`](apps/web/src/dashboard/selection.ts) با تستِ واحد.
- **معیار پذیرش:** ✅ **در مرورگر با api اثبات شد** — ساختِ بورد (باز شدنش در ۸٫۴)؛ **تغییرِ نقشِ عضو
      (member→admin) در UI بازتاب یافت** (دعوت→پذیرش با کاربرِ دوم→تغییرِ نقش→refetch). دو باگِ زنده هم
      رفع شد: تصادمِ مسیرِ SPA با پروکسی (`/team` مفرد) و refreshِ دوگانه‌ی StrictMode.
- **معیارِ پذیرشِ فولدر/سطل:** ✅ حلقه‌ی کامل در مرورگر — فولدر «طرح‌ها» ساخته شد، بوردی به آن رفت
      و در فیلترِ فولدر فقط همان دیده شد؛ فولدر تغییرِ نام یافت و حذف شد (بوردش پاک نشد، به «همه» رفت)؛
...
### گام ۸٫۵ — ★ رفعِ سه یافته‌ی M2 (M3-D4) — ✅ **۳ و ۲ اعمال شد، ۴ موکول** (۱۴۰۵/۰۶/۱۰)
...
> اعمال کن و مثلِ B-1 در PROGRESS ثبت کن.  **مالک ۱۴۰۵/۰۶/۱۰ یافته‌ی ۳ و ۲ را تایید کرد؛ ۴ را نه.**
- [x] **یافته‌ی ۳ — مکان‌نمای زنده‌ی همتا** ✅ (لمسِ **M1** `HamboomCanvas` — ثبت در PROGRESS):
      propِ `onPointerUpdate` به `HamboomCanvas` افزوده شد که `onPointerUpdate`ِ موتور (که **مختصاتِ
      صحنه** می‌دهد) را forward می‌کند — پس صفر تکرارِ فرمولِ پیکسل→صحنه (ADR-024). `BoardPage` مکان‌نما
      را emit می‌کند، `applyPeers` را می‌گیرد، و `PeerCursors`ِ M1 را با `sceneToOverlayPixel` رندر می‌کند.
      **مرورگر: مکان‌نمای همتا با نام/رنگ ظاهر و زنده حرکت کرد؛ رسم هم همگام.**
- [!→موکول] **یافته‌ی ۴ — انزوای ویرایشگرِ متن:** ۴ج (مستند و موکول). **مالک ۱۴۰۵/۰۶/۱۰ رد کرد.**
      رفعِ واقعی یا پرریسک است (۴الف: داخلیِ موتور) یا تصمیمِ UX (۴ب: قفلِ نرم)؛ لبه‌ی نادر و با
      `e2e/text-latency.spec.ts` ثبت‌شده. وقتی UXِ متن تمرکز شد بازبینی شود.
- [x] **یافته‌ی ۲ — opِ اضافیِ `schemaVersion`** ✅ (لمسِ **M2** `adapter` — ثبت در PROGRESS):
      مهرِ `schemaVersion` **تنبل** شد — `BoardPage` یک `Y.Doc`ِ ساده می‌دهد و adapter فقط روی اولین
      نوشتنِ واقعی و **اگر بی‌نسخه باشد** مهر می‌زند (در `meta`، بیرونِ undo). تستِ خودآزمون در
      `adapter.test.ts` (با شکستِ عمدی قرمز شد).
- **معیار پذیرش:** ✅ هر یافته‌ی اعمال‌شده با تست قفل شد؛ هیچ‌کدام بدونِ تاییدِ صریحِ مالک اعمال نشد؛
      لمسِ M1/M2 در PROGRESS ثبت شد. `pnpm verify` سبز.
...
### گام ۹٫۱ — نوار ابزار ✅ (۱۴۰۵/۰۶/۱۲)
- [x] نوارِ عمودیِ فشرده (RTL، logical properties): انتخاب/دست/استیکی/متن/شکل/کانکتور/قلم/
      تصویر/فریم/کامنت/پاک‌کن — با پالتِ استیکیِ ۱۲رنگه (PLAN §۷٫۳)؛ میان‌برهای صفحه‌کلید
      (`toolForShortcut`)؛ **ابزارِ فعال در کانالِ حضور** (`emitActiveTool`→`PeerState.activeTool`).
      ⚠️ کامنت `comingSoon` (فاز ۱۰ موکول)؛ لیزر در `HB_TOOLS` نیست (بعداً). **تصویر به فاز ۱۱٫۲
      موکول** (adapter بدونِ `assets`؛ وایرینگ آماده، نوتیسِ گراسفول تا آن‌وقت).
- [x] ★ P6: مختصاتِ بوم **آینه نمی‌شود** — نگاشتِ ابزار فقط `setActiveTool` می‌کند؛ جای‌گذاریِ نوار
      با logical properties (`inset-inline-start`) خودِ RTL را می‌دهد، بی‌آینه‌کردنِ مختصات.
- [x] از ابزارهای `canvas-core` استفاده شد، بازسازی نشد (ADR-024): `HB_TOOLS`/`toolForShortcut`/
      `createStickyTool`/`ImageTool`/`DrawTool`/`createShape…Frame`/`HB_STICKY_PALETTE`. تنها لمسِ نو،
      propِ افزایشیِ `orientation` روی `Toolbar` (تاییدِ مالک ۱۴۰۵/۰۶/۱۲؛ در PROGRESS مثلِ B-1 ثبت).
- **معیار پذیرش:** ✅ در دو مرورگرِ همگام: استیکیِ فارسی («استیکی فارسی») + کانکتور + فریم از نوار
      ساخته و **همگام** شدند؛ ابزارِ فعالِ همتا **دوطرفه** نمایش یافت (فریم/انتخاب/قلم)؛ Stylelint
      هیچ propertyِ فیزیکی نگرفت. `pnpm verify` سبز.
- [x] **پولیش: نوار ابزارِ نیتیوِ excalidraw پنهان شد** (تاییدِ مالک ۱۴۰۵/۰۶/۱۲): propِ افزایشیِ `hideNativeUI`
      روی `HamboomCanvas` (لمسِ M1ِ دوم) با CSS نوار/منو/فوترِ نیتیو را می‌بندد (نه zenMode)؛ `ZoomControl`ِ
      canvas-core (**reuse**) جای فوترِ زوم را می‌گیرد و به bottom-end می‌رود تا با نوارِ عمودی تصادم نکند
      (undo با `Ctrl+Z`). در مرورگر اثبات شد (نوار/فوتر ناپدید، رسم + زوم/fit سالم). `pnpm verify` سبز.
- [x] **کاملیتِ نوار: بیضی/لوزی/خط/لیزر** (تاییدِ مالک ۱۴۰۵/۰۶/۱۲): با پنهان‌شدنِ chromeِ نیتیو، ابزارهایی که فقط
      آنجا بودند به نوارِ ما آمدند تا رگرسیون نشود. **شکل/کانکتور فلای‌اوتِ واریانت** (مستطیل/بیضی/لوزی · پیکان/خط —
      سمتِ اپ، بی‌لمسِ M1) + **لیزر** (لمسِ M1ِ سوم: `laser` در `HB_TOOLS` + آیکون + `tool.laser`؛ رندرِ محلی/همتا روی
      روکش با `emitEphemeral`). embed (P2)/library (منبعِ خارجی)/keep-active (modifier) عمداً کنار. اثباتِ دو-تب:
      بیضی/لوزی/خط سینک (خط با pointsِ سالم، نه جعبه)، لیزرِ همتا رندر شد. ⚠️ **کانالِ ephemeral برای اولین بار
      end-to-end با سرورِ واقعی اثبات شد.** `pnpm verify` سبز.
- [x] **استروکِ زنده‌ی قلمِ همتا** (بعد از لیزر، تاییدِ مالک ۱۴۰۵/۰۶/۱۲): `redrawOverlay` حالا `kind:"draw-stroke"`
      را هم می‌کشد (رنگ/عرضِ خودِ استروک × zoom). همتا هم استروکِ **زنده** (روکش) هم **نهایی** (صحنه) را می‌بیند.
      اثباتِ دو-تب: روکشِ tab-1 حین استروکِ seed alpha=۲۵۵. `pnpm verify` سبز.
...
### گام ۱۱٫۱ — ★★ سقفِ حافظه → فاز ۳ی ADR-006 (room affinity) — ✅ **ثبتِ trigger** (۱۴۰۵/۰۶/۱۳)
- [x] **تصمیمِ مالک (۱۴۰۵/۰۶/۱۳): ثبتِ trigger، نه پیاده‌سازی.** [ADR-048](ARCHITECTURE_DECISIONS.md#adr-048)
      affinity را به **M5** (زیرساخت) موکول کرد **با تریگرِ عددی**: پایه ۷۶MB/بوردِ ۵۰۰۰عنصری → ظرفیتِ نود
      حافظه‌محور (heapِ ۴GB ≈ ~۴۰ بورد)؛ فاز ۳ فعال می‌شود وقتی **≥۲ رِپلیکا** + حافظه‌ی مقیمِ یک نود **>~۶۰٪ heap**.
      دلیل: M3 تک‌نود/لوکال (P3)، `sessionAffinity` مالِ Service/K8s = M5؛ «shardingِ زودهنگام = باگ» (خودِ ADR-006).
- **معیار پذیرش:** ✅ ADR-048 آستانه را با **عدد** ثبت کرد (نه «بعداً»)؛ در `m4-handoff` به‌عنوان ارثیه‌ی M5 می‌آید.

### گام ۱۱٫۲ — نقطه‌ی ادغام (smoke کامل) + تحویلِ M4
- [x] سناریو در مرورگرِ واقعی اثبات شد (تکه‌تکه، این نشست + پیش‌تر): ورود با OTP ✓، بورد ✓،
      **آپلود + نمایشِ تصویر از storageِ واقعی** ✓ (گام ۱۱٫۲، این نشست)، نوار ابزار ✓ (فاز ۹).
      همگامیِ **دو مرورگر** در فاز ۸٫۴/۸٫۵ در مرورگر اثبات شده و سمتِ سرور با سنجه‌ی `rt:cluster`
      (دو پروسه‌ی واقعی روی یک Redis) تایید است. ⚠️ یک اجرای **یکجای** دو-تبی این نشست نشد (استکِ
      dev سالم و بالا است — دموی زنده در دسترس).
- [x] ✅ **۵ README** (api/auth-core/storage/sdk/web) + `PROGRESS-M3` به‌روز + **`docs/m4-handoff.md`**
      (نقطه‌ی ورودِ billing: team/subscription، جدول‌های plans/subscriptions/payments/invoices، و ارثیه‌های
      به‌تعویق‌افتاده: فاز ۱۰، `CommentPin`، `apps/worker`، room affinity→ADR-048، یافته‌ی ۴ متن).
- [x] ✅ `pnpm verify` سبز (هر ۸ گیت)؛ **هر ۷ سنجه‌ی زنده سبز** (روی postgres+redis+MinIOِ واقعی).
- **معیار پذیرش:** ✅ اجزای سناریو در مرورگرِ واقعی کار می‌کنند؛ verify سبز؛ ۷ سنجه سبز؛
      `m4-handoff.md` مثلِ `m3-handoff.md` خودکفا است.
