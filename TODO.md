...
- [!] **G-2 (از گام ۱٫۴): بلوکه — نیازِ تصمیمِ مالک برای افزودنِ Playwright.**
      تست رگرسیون هش پیکسلی جهت متن. [ADR-025](ARCHITECTURE_DECISIONS.md#adr-025) روی
      wrapper `fillText` بنا شده و **بی‌صدا** می‌شکند اگر موتور مسیر رندر را عوض کند؛
      تشخیصِ فعلی فقط شمارنده‌ی دستی در `#spike` است. **چرا بلوکه:** (۱) Playwright یک
      وابستگیِ سنگین (باینریِ مرورگر) است که مالک در گام ۱٫۴ عمداً برای یک تست به تعویق
      انداخت — افزودنش تصمیمِ اوست؛ (۲) این محیطِ خودکار رندرِ واقعی/composite ندارد، پس
      نمی‌توانم خودِ تستِ پیکسلی را اینجا **تایید** کنم (اصلِ «چیزی که تایید نشده را
      انجام‌شده ننویس»). آماده برای وقتی مالک وابستگی را تایید کند.
