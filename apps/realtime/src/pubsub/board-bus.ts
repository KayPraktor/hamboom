...
export const BUS_KINDS = {
  ...
  SAVED: 3,
  /**
   * ★★ «این بورد را باز دارید؟ حالتتان را بدهید» — **یافته‌ی F-2**،
   * [ADR-041](../../../../ARCHITECTURE_DECISIONS.md#adr-041).
   *
   * نودی که تازه اتاقی را باز می‌کند این را می‌فرستد؛ هر نودی که همان اتاق را
   * باز دارد با یک `UPDATE`ِ **حالتِ کامل** جواب می‌دهد.
   *
   * ⚠️ **چرا لازم است:** خواندن از Postgres فقط چیزی را می‌آورد که **پایدار
   * شده**. بینِ مرگِ نودِ صاحب و انقضای اجاره‌ی قفل هیچ نودی صاحب نیست و در آن
   * پنجره هیچ updateی پایدار نمی‌شود — تنها نسخه‌اش حافظه‌ی نودی است که گرفتتش.
   * نودی که آن پیامِ گذرگاه را از دست بدهد، بدونِ این پرسش **هرگز** به آن
   * نمی‌رسد: گذرگاه گذشته را نگه نمی‌دارد و دیتابیس چیزی ندارد.
   *
   * ★ `payload` خالی است. جواب از نوعِ `UPDATE` می‌آید، نه یک نوعِ تازه — چون
   * مسیرِ اعمالش دقیقاً همان است و Yjs idempotent.
   */
  STATE_REQUEST: 4,
} as const;
