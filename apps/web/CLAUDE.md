...
- **★ لیزر:** ابزارِ موتور `setActiveTool("laser")` (اینرت)، و دنباله از `onPointerUpdate`ِ صحنه‌ای به‌صورت
  ...
  می‌کرد ولی هیچ‌وقت peer-to-peer دیده نشده بود؛ با console-probe + pixel-detector در دو تب).
- **★ استروکِ زنده‌ی قلمِ همتا هم رندر می‌شود:** `redrawOverlay` علاوه بر لیزر، `kind:"draw-stroke"` را هم می‌کشد
  (رنگ/عرضِ **خودِ استروک**؛ عرض × zoom چون در واحدِ صحنه است). draw-tool از قبل ephemeral را emit می‌کرد؛ فقط رندرِ
  گیرنده نبود. پس همتا هم استروکِ **زنده** (روکش) هم **نهایی** (صحنه) را می‌بیند. اثبات: روکشِ tab-1 حین استروکِ seed alpha=۲۵۵.
