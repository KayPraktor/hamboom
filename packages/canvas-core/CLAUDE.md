...
| ★★ **لایه‌ای که از مختصاتِ بوم پروجکت می‌کند نباید property منطقی برای مبدأ داشته باشد** | در سندِ `dir="rtl"`، `inset-inline-start: 0` می‌شود `right: 0` و عنصر به اندازه‌ی `عرضِ لایه − عرضِ عنصر` پرت می‌شود. **`transform` درست است**، پس تستِ واحد سبز می‌مانَد — jsdom **layout ندارد** و فقط رشته‌ی transform را می‌سنجد. سنجیده در M2 گام ۳٫۷: `transform.x = 220` ولی `left`ِ واقعی ۷۷۹٫۴ | روی خودِ **لایه** `direction: ltr` بگذار (استثنای بومِ ADR-016؛ `left`ِ خام را گیتِ Stylelint می‌گیرد) و اگر متنِ فارسی داخلش هست، روی همان متن `direction: rtl` برگردان. الگوی دوم برای مقدارِ **محاسبه‌شده در JS**: `left` را inline در کامپوننت بگذار — همان کاری که `ContextMenu` می‌کند |
...
| **StrictMode + APIِ امریِ Excalidraw ناسازگارند** ([ADR-028](../../ARCHITECTURE_DECISIONS.md#adr-028)) | StrictMode موتور را دوبار mount/unmount می‌کند؛ بعدش `onChange → setSnapshot` مرده می‌مانَد و پنل با **هیچ** انتخابِ موتوری به‌روز نمی‌شود (منطقِ refreshCounts درست است). لغوِ listener درمانش نکرد | دمو در `<StrictMode>` نیست (`dev/main.tsx`). **ریسکِ M3:** apps/web قبل از StrictMode باید الگوی امن را probe کند — ADR-028 |
...
| **کلیپ‌بوردِ سیستم بینِ تست‌ها مشترک است و نوشتنش async است** | `Ctrl+X` بعد `Ctrl+V` گاهی **۲ عنصر** برمی‌گرداند نه ۳: موتور روی همان Ctrl+X کلیپ‌بورد را async با فرمتِ خودش بازنویسی می‌کند، و در آن پنجره paste هنوز **نشانه‌ی تستِ قبلی** را می‌بیند → مسیرِ «متنِ خارجی» → یک استیکی. **باگِ محصول نیست** (هیچ انسانی این‌قدر سریع تایپ نمی‌کند)؛ ولی flakeِ ~۱ در ۵ روی **اجرای سرد** می‌سازد | قبل از `Ctrl+V` صریح صبر کن تا کلیپ‌بورد محتوای داخلی داشته باشد (`waitForInternalClipboard` در [`e2e/clipboard.spec.ts`](e2e/clipboard.spec.ts)) |
| **مهلتِ ثابت (`waitForTimeout`) روی اجرای سرد کم می‌آید** | تست به‌جای رفتار، سرعتِ ماشین را می‌سنجد؛ اولین اجرا بعد از پاک‌شدنِ کشِ Vite می‌افتد | همیشه `expect.poll` روی یک **شرط**، نه مهلتِ ثابت. برای بازتولیدِ حالتِ سرد: `rm -rf node_modules/.vite` |
...
- ★ **`laser` به `HB_TOOLS` اضافه شد** (M3 گام ۹٫۱ کاملیت، تاییدِ مالک ۱۴۰۵/۰۶/۱۲): `ToolId`/`HB_TOOLS` (میانبرِ `l`) +
