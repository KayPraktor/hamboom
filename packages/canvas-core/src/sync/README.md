...
- **re-projectِ حضور با هر تغییرِ viewport** — شاملِ **panِ خالص** و zoom. ★ درسِ
  ثابت‌شده در دمو (۱۴۰۵/۰۵/۰۷): لایه‌ی حضور را از **نمای معتبرِ `onScrollChange`**
  پروجکت کن، نه از `api.getAppState()` که درست بعد از یک جابه‌جاییِ نما **یک فریمْ
  کهنه** است؛ و با هر تغییرِ viewport re-render بده. در دمو، panِ خالص مکان‌نمای همتا
  را سرِ جای صفحه جا می‌گذاشت چون render مقدارِ scroll را کهنه می‌خواند — با state
  واحدِ viewport از `onScrollChange` حل شد.
