 ...
 *
 * ── ⚠️⚠️ وضعیت: مرحله‌ی ۵ **امروز قرمز است، و عمداً همین‌طور مانده** ────
 *
 * مرحله‌های ۱ تا ۴ در هر اجرا سبزند. مرحله‌ی ۵ (همگرایی بعد از قطعی) در ۳ اجرا
 * از ۴ **شکست می‌خورد** — و علتش یک باگِ واقعیِ خوشه است، نه تست: **یافته‌ی
 * F-2** در [PROGRESS.md](../../../PROGRESS.md).
 *
 * خلاصه‌اش: بینِ مرگِ نودِ صاحب و انقضای اجاره‌ی قفل (تا ۳۰ ثانیه) **هیچ نودی
 * صاحب نیست**، پس updateِ کلاینت هیچ‌جا پایدار نمی‌شود؛ تنها نسخه‌اش حافظه‌ی
 * همان نودی است که گرفتتش. نودی که آن پیامِ گذرگاه را نگیرد، هرگز به آن
 * نمی‌رسد — نه از گذرگاه (pub/sub گذشته را نگه نمی‌دارد) و نه از Postgres
 * (چیزی نوشته نشده).
 *
 * ★ **این تست عمداً «رفع» نشده تا سبز شود.** قرمزیِ آن مدرکِ باگ است؛ تصمیمِ
 * راه‌حل با مالک است (سه گزینه در PROGRESS).
 *
 * اجرا:
