 ...
 * **بر اساسِ نتیجه‌ی همین probe** در `ydoc-schema` نوشته شد.
 *
 * ⚠️ **این فایل دیگر مرجعِ رفتار نیست.** نگهبانِ دائمیِ وفاداری
 * [`element-codec.test.ts`](element-codec.test.ts) است که codecِ **واقعی** را
 * می‌آزماید. آنچه اینجا می‌مانَد و جای دیگری تکرارپذیر نیست، اندازه‌گیریِ
 * **ضدِ ادعا** است: نوشتنِ کلِ عنصر — کاری که codecِ محصولی عمداً بلد نیست.
 */
