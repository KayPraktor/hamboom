 ...
 *
 * مصرف‌کننده‌ی **نوشتن**: فقط `scripts/backup-db.ts`، `scripts/backup-storage.ts` و
 * `scripts/restore-drill.ts`.
 *
 * ⚠️ **از M6 فاز ۷٫۴ این ادعا فقط برای نوشتن درست است:** `GET /admin/system` باکتِ پشتیبان را
 * **می‌خوانَد** (سنِ آخرین dump/mirror) و نامش را از `adminProbeEnvSchema` می‌گیرد، نه این‌جا.
 * ⊕ و یک صداقتِ لازم: api از قبل همان کلیدهای S3 را داشت، پس تا آن روز فقط **نام** را نداشت،
 * نه دسترسی را — «کمترین دسترسی» این‌جا یعنی کمترین **دانستن**، نه یک مرزِ رمزنگارانه.
 */
...
/**
 * ── ★★ probeِ وضعیتِ سیستمِ پنل (M6 فاز ۷٫۴، ADR-067) ────────────────────
 *
 * ⚠️⚠️ **هر دو متغیرِ اتصال عمداً اختیاری‌اند — و این برعکسِ `AUDIT_RETENTION_DAYS` است.**
 * آن یکی یک سیاستِ **مخرب** بود (داده چند روز بعد پاک شود) و حدس‌زدنش خطرناک، پس بدونِ عدد
 * بالا نمی‌آید. این‌ها ورودیِ یک **چکِ سلامت**‌اند: `REDIS_URL` در `redisEnvSchema` پیش‌فرض
 * ندارد، پس افزودنِ مستقیمِ آن بخش به `apiEnvSchema` یعنی api **همه‌جا** بدونِ آن بوت نمی‌شود —
 * `.env.example`ی که CI از رویش می‌سازد، انکرِ `x-api-env`ِ compose، و هر محیطِ قدیمی. برای یک
 * چکِ سلامت، «پیکربندی نشده» جوابِ درست است، نه مرگِ سرور.
 *
 * ⚠️ **`S3_BUCKET_BACKUPS` این‌جا عمداً پیش‌فرضِ `backupEnvSchema` را به ارث نمی‌برد.** یک نامِ
 * حدسی روی محیطی که باکتش نامِ دیگری دارد، چکِ سلامت را **قرمزِ دروغین** می‌کند؛ و یک هشدارِ
 * دروغین بدتر از نبودِ هشدار است، چون اپراتور یاد می‌گیرد نادیده‌اش بگیرد.
 *
 * ⚠️ **تله‌ی ترکیب:** `REDIS_URL` این‌جا اختیاری و در `redisEnvSchema` اجباری است. این دو بخش
 * هرگز نباید با `.and()` به هم برسند (api فقط این یکی را دارد، realtime فقط آن یکی) — وگرنه
 * اشتراکِ تایپ سخت‌گیرانه‌تر می‌شود و api دوباره بدونِ Redis بوت نمی‌شود.
 */
export const adminProbeEnvSchema = z.object({
  /** نبودنش ⇒ چکِ Redis «نمی‌دانم» می‌شود، نه «خراب». */
  REDIS_URL: z.string().min(1).optional(),
  REDIS_TLS: envBoolean("false"),
  /** نبودنش ⇒ چکِ باکتِ پشتیبان و سنِ پشتیبان «نمی‌دانم» می‌شوند. */
  S3_BUCKET_BACKUPS: z.string().min(1).optional(),
  /**
   * مهلتِ **هر** probe. اندازه‌گیریِ ۷٫۰: PINGِ زنده ۱۵٫۸ms، پورتِ بسته ۲٫۶ms، و میزبانِ
   * بی‌پاسخ **تا خودِ مهلت** طول می‌کشد — پس این عدد است که زمانِ بدترین حالت را می‌بندد،
   * نه کدِ خطا. probeها موازی‌اند، پس سقفِ کلِ درخواست هم تقریباً همین است.
   */
  ADMIN_SYSTEM_TIMEOUT_MS: envInt(2_000),
  /** آستانه‌ی «پشتیبانِ کهنه» — عددِ سیاستیِ تاییدشده‌ی فاز ۰: ۳۰ ساعت. */
  ADMIN_BACKUP_STALE_HOURS: envInt(30),
});
export type AdminProbeEnv = z.infer<typeof adminProbeEnvSchema>;


/**
 * ── ★ نگهداشتِ داده (M5 گام ۸٫۱، تصمیمِ M5-D9) ───────────────────────────
 *
 * «چند روز بعد از انداختن در سطل، بورد واقعاً پاک شود؟» — **۳۰ روز**، تصمیمِ مالک
 * (۱۴۰۵/۰۶/۱۹). این عدد **سیاست** است نه پارامترِ فنی: به انتظارِ کاربر و تعهدِ ما
 * بسته است. به همین دلیل تا پیش از این تاریخ `purge-deleted.ts` اصلاً پیش‌فرض نداشت
 * و بدونِ `--days` بالا نمی‌آمد؛ حالا پیش‌فرض همان تصمیم است، نه حدسِ ما.
 *
 * ⚠️ عوض‌کردنِ این عدد یعنی عوض‌کردنِ تعهد به کاربر — تصمیمِ مالک است، نه تنظیمِ اپراتور.
 * مصرف‌کننده: فقط `scripts/purge-deleted.ts`.
 */
export const retentionEnvSchema = z.object({
  /** روزهای ماندن در سطلِ بازیافت پیش از حذفِ نهایی (M5-D9: ۳۰). */
  TRASH_RETENTION_DAYS: envInt(30),
});
