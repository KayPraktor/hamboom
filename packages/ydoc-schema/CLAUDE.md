...
| `src/error-codes.ts` | کدهای `HB_ERROR` — تکه‌ای از ۲٫۴ که زودتر لازم شد | ۲٫۳ ✅ |
| `src/migrations/` | موتور + رجیستریِ ترتیبی + `checkClientVersion` | ۲٫۳ ✅ |
| `src/protocol.ts` | کدهای پیامِ PLAN بخش ۵٫۳ | ۲٫۴ |

## افزودنِ یک migration

۱. `SCHEMA_VERSION` در [`doc.ts`](src/doc.ts) را یکی بالا ببر · ۲. یک ورودی با
`to = from + 1` به **انتهای** [`migrations/registry.ts`](src/migrations/registry.ts)
اضافه کن · ۳. تابع را **تنها روی خودِ سند** بنویس (بدونِ I/O، بدونِ زمانِ جاری —
ممکن است ماه‌ها بعد اجرا شود و باید همان نتیجه را بدهد) · ۴. ورودیِ منتشرشده را
هرگز ویرایش نکن.

`migrateDocument` خودش زنجیره را بازرسی می‌کند: پرشِ نسخه، گسستگی، و **migrationِ
اضافه‌شده بدونِ بالابردنِ `SCHEMA_VERSION`** هر سه خطا می‌دهند. آن آخری رایج‌ترین
اشتباه است و بدونِ نگهبان، migration **هرگز اجرا نمی‌شد** و کسی نمی‌فهمید.
