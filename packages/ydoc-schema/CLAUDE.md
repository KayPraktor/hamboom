...
| `src/protocol.ts` | هر ۷ کدِ پیامِ PLAN بخش ۵٫۳ + framing | ۲٫۴ ✅ |

## ★★ دو قاعده‌ی پروتکل که نباید شکسته شوند

۱. **نوعِ پیامِ ناشناخته بی‌صدا نادیده گرفته می‌شود** (`decodeMessage` → `null`)، و
   **بایتِ اضافه در انتهای پیام خطا نیست**. این دو با هم یعنی نسخه‌ی بعدی می‌تواند
   هم پیامِ تازه اضافه کند و هم به پیامِ موجود فیلد **اضافه** کند، بدونِ شکستنِ
   کلاینت‌های باز. در یک اپِ وب که کاربر تبش را هفته‌ها باز می‌گذارد، «کلاینتِ
   قدیمی» یعنی بخشِ بزرگی از کاربران.
   ⚠️ ولی پیامِ **خرابِ** نوعِ شناخته‌شده `ProtocolError` می‌دهد. ناشناخته یعنی «هنوز
   بلد نیستم»، خراب یعنی «یک طرف باگ دارد» — قاطی‌کردنشان باگ‌های framing را برای
   همیشه پنهان می‌کند.
۲. ★ **fail closed:** نقشِ ناشناخته → `viewer` و وضعیتِ ذخیره‌ی ناشناخته →
   `unsaved`. انداختنِ پیامِ `HB_PERMISSION` یعنی کاربری که همین الان تنزل داده شده
   با دسترسیِ قبلی ادامه می‌دهد — یک حفره‌ی مجوز.

⚠️ **`BOARD_ROLES` و `SAVE_STATUSES` با ایندکس روی سیم می‌روند** — فقط می‌شود به
**انتهایشان** اضافه کرد. جابه‌جا کردنشان یعنی کلاینتِ قدیمی نقشِ اشتباه می‌فهمد.
